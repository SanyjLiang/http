...
int main(int argc, const char *argv[])
{
    ...
    while (1)
    {
        ...
        socklen_t socklen = sizeof(peer);       //接收长度

        //接收客户端的连接请求
        //返回的套接字就是服务器端创建处理的用于跟客户端进行套接字文件描述符
        int sock = accept(lis_socket, (struct sockaddr *)&peer, &socklen);
        if (sock == -1)
        {
            perror("accept error");
            return -1;
